...
class P2BTest {
    ...
    fun dreamFourCheckDetails() {
        ...
        onView(withId(R.id.last_updated))
            .check(matches(withPattern("Last updated 2023-09-10 at ..:12:13 [AP]M")))
    ...
    fun dreamFourRotateClickDeferThenBackRotateAndConfirmList() {
        ...
        checkEntryGone(2)
        onView(withId(R.id.last_updated))
            .check(matches(not(withPattern("Last updated 2023-09-10 at ..:12:13 [AP]M"))))
        pressBack()
        scenario.recreate()
        onView(withId(R.id.dream_recycler_view))
            .check(
                    ...
                    atPosition(
                        ...
                        allOf(
                            ...
                            hasDescendant(
                                allOf(
                                    withId(R.id.list_item_image),
                                    matchesDrawable(R.drawable.ic_dream_fulfilled),
                                    withEffectiveVisibility(Visibility.VISIBLE)
                                )
                            )
                        )
    ...
    fun dreamEightUncheckDeferredCheckFulfilledConfirmLastUpdated() {
        ...
        onView(withId(R.id.last_updated))
            .check(matches(withPattern("Last updated 2023-09-10 at ..:12:13 [AP]M")))
        val timestamp0 = getText(withId(R.id.last_updated))
        changeCheckboxState(CHECKBOX.DEFERRED, false)
        val timestamp1 = getText(withId(R.id.last_updated))
        assertNotEquals(timestamp0, timestamp1) //######
        sleep(1000)
        changeCheckboxState(CHECKBOX.FULFILLED, true)
        val timestamp2 = getText(withId(R.id.last_updated))
